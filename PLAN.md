...
- [x] Confirm `podman-compose -f infra/podman-compose.yml up -d` brings up all 4 infra containers healthy, and `CreateTicket` over Connect writes a real Mongo document

**Definition of done**: ✅ **met 2026-07-23.** `podman-compose up -d` gives healthy Mongo/Redis/Qdrant/MinIO, and a `buf curl` call to `backend-services` `CreateTicket` inserted a real ticket (`tkt_01KY7MN582S7C4FMVFSGGGQDWC`) confirmed via `mongosh` and re-readable via `ListTickets` from the compose-launched instance — no Python and no frontend yet.
...
- **2026-07-23: `podman-compose up` fully verified, resolving the earlier WSL2 blocker.** `wsl --status` had misleadingly reported WSL2 as unsupported — that message was actually about the unrelated legacy WSL1, and WSL2 itself was already installed and working. Real remaining issues, in order: (1) `podman machine init` + `start` failed with `ssh error: machine not in running state` — root cause was WSL2 mounting legacy **cgroup v1** (hybrid) while modern `systemd` (which Podman's WSL machine nests via `unshare` inside the distro) requires **cgroup v2** (unified); fixed by adding `kernelCommandLine = cgroup_no_v1=all` under `[wsl2]` in `%USERPROFILE%\.wslconfig` (didn't exist yet) + `wsl --shutdown` to restart the whole WSL2 VM (affects *all* distros, not just Podman's). (2) No compose provider was installed (`podman compose` needs `docker-compose` or `podman-compose` on PATH) — installed via `pip install podman-compose` (Python already present) rather than a separate binary. (3) `podman-compose` (v1.6.0) has a real path-resolution bug building services with a `build:` `context:`/`dockerfile:` combo on Windows — it does not resolve them relative to the compose file's directory like `docker compose` does, and neither CWD-relative nor compose-relative rewrites produced a working `-f` flag. Workaround (not a bug to keep re-debugging): pre-build the image with `podman build` / `make build-backend-image`, then reference `image: localhost/servicesphere/backend-services:latest` in `infra/podman-compose.yml` instead of an inline `build:` section — `podman-compose up -d` then just runs it. (4) `infra/containerfiles/backend-services.Containerfile` had gone stale after the RPC-per-collection restructure — still pinned `golang:1.23-alpine` (our actual `go.mod` needs `go 1.26.5`, matching the locally installed toolchain) and still built `./data-access/cmd/server` (that path no longer exists; `main.go` is at the module root) — fixed both.
- PowerShell note: `buf curl -d '{"json":"inline"}'` reliably mangles embedded double quotes when passed to a native exe — write the JSON to a file and use `-d "@<path>"` instead.
