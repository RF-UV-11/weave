...
- [x] Core `ChatSession`/`ChatMessage`: proto, Mongo collections, RPCs — session memory persisted through `core` only, never held by `orchestrator` directly (`docs/architecture/ARCHITECTURE.md` §5)
- [x] `orchestrator/server/session_memory.py` loads prior turns for a session before generating (`resolve_session`) and persists both sides of the turn after (`persist_turn`), giving real multi-turn context instead of every `ChatStream` call being independently stateless. Fails soft (a core outage or stale `session_id` degrades to "no prior context," never fails the turn) — same posture as rate limiting's fail-open (`docs/architecture/SECURITY.md` §5)
- [x] Long-term semantic memory: `core` gets a Qdrant client (one collection per tenant, `docs/architecture/SECURITY.md` §2) and `Upsert`/`Search` memory RPCs; `orchestrator/server/semantic_memory.py` computes the embedding (via Ollama's `nomic-embed-text`) and calls `core` — `core` remains the only tier holding real infra credentials, `orchestrator` never talks to Qdrant directly
...
**Notes**:
- **A real design decision, not an oversight**: on the guardrail-buffered path (`chat_service.py`'s `_guarded_response`), session history persists whatever was actually *sent* to the caller, not the model's raw pre-screening generation. If a guardrail redacts an answer, persisting the raw (blocked) text into session history would let it quietly leak back into a *later* turn's context once replayed — `resolve_session()` doesn't re-screen replayed history, it trusts it. Persisting the post-redaction text (the real `GUARDRAIL_REFUSAL` string on a violation) closes that gap at the point memory is written, rather than needing every future turn to re-discover the same violation.
- **Only `user`/`assistant` messages are replayed into a future turn's context** (`session_memory.py`'s `_REPLAYABLE_ROLES`) — a `tool` role message is the raw result of one specific tool call, already folded into that turn's assistant answer; replaying it verbatim into later turns would bloat context with stale tool output instead of the synthesized answer that already captures what mattered from it.
- `GetSessionMessages` sorts by `_id` (a ULID), not `created_at` — `created_at` is RFC3339-second resolution, so messages appended within the same second (routine, not an edge case, since a tool-call turn appends both the user and assistant message back-to-back) would tie and sort unpredictably.
- 7 new tests in `orchestrator/tests/test_session_memory.py` covering session creation, history replay (with `tool`-role filtering), and fail-soft behavior on both `CreateSession` and `GetSessionMessages`/`AppendMessage` errors.

**Semantic memory definition of done**: ✅ **met.** Live-verified cross-*session* recall specifically (two entirely separate `session_id`s, unlike the session-memory verification above, to isolate that this is the long-term tier doing the work, not short-term replay): session A said "my favorite product is the Acme Wave Headphones, SKU-WH-9"; session B, a brand-new conversation with no shared session state, was asked "What's my favorite product?" and correctly answered with the Wave Headphones/SKU-WH-9 — proving `recall()` actually retrieved and injected the fact into a completely independent turn's context.

**Notes**:
- **Deliberate scope simplification, not an oversight**: every user turn's raw message is stored as one memory point verbatim — there's no LLM-driven fact extraction (distilling "the user's name is Jordan" out of a longer message) and no deduplication. This was enough to make cross-session recall genuinely work end-to-end (see above), which was the actual goal; a real fact-extraction/summarization pass is a natural follow-up once there's a reason to care about memory volume or precision, not a blocker to shipping a working feature.
- **Qdrant point ids are UUIDs, not this codebase's usual ULID convention** (`core/qdrant/client.go`) — a real constraint of the store (Qdrant point ids must be a `uint64` or a UUID), discovered while implementing, not a stylistic choice.
- **Test design pitfall caught before it shipped**: the first draft of `core/qdrant/client_test.go` used scaled-but-parallel vectors (`[1.0, 1.001, ...]` vs `[50.0, 50.001, ...]`) to represent "near" vs "far" memories. Cosine distance (what the collection is configured with) is angle-based, not magnitude-based, so two parallel vectors score almost identically regardless of scale — the test would have been flaky/meaningless. Switched to orthogonal one-hot vectors (cosine similarity exactly 0 between "far" pairs, exactly 1 for an identical "near" pair) before this was ever run for real, not caught by CI after the fact.
- Both `recall()` and `remember()` fail soft, same posture as `session_memory.py` — an Ollama/Qdrant outage degrades a turn to "no long-term recall," never fails the chat request.
- 6 new orchestrator tests (`test_semantic_memory.py`) plus 2 for the new `embed()` wrapper (`test_ollama_client.py`).
